...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch_id, batch in enumerate(train_dataloader):
            ...
            optimizer.step()
            print(f'cuda device usage (after batch {batch_idx+1}): {torch.cuda.memory_allocated() / 2**20}')
            ...
            print(f'Epoch: {epoch+1}, Batch: {batch_idx+1}, From torchmetric: {train_mse.compute()} From criterion: {loss.item()}')
