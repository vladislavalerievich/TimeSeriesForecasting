...
def plot_multivariate_timeseries(
    ...
    if start is not None and frequency is not None:
        ...
        if pred_len > 0:
            ...
            future_dates = pd.date_range(
                start=next_timestamp, periods=pred_len, freq=pandas_freq
            )
    else:
    ...
    for i, ax in enumerate(axes):
        ...
        if future_values is not None:
            ax.plot(
                future_dates, future_values[:, i], color="blue", label="Ground Truth"
            )
...
def plot_from_container(
    ...
    start_ts = (
        batch.start if not hasattr(batch.start, "__len__") else batch.start[sample_idx]
    )
    frequency = (
        batch.frequency
        if not hasattr(batch.frequency, "__len__")
        else batch.frequency[sample_idx]
    )
    ...
    # Handle case where predicted_values is already for a single sample or is a batch
    if predicted_values is not None:
        if predicted_values.ndim >= 3 or (
            predicted_values.ndim == 2
            and predicted_values.shape[0] > future_values.shape[0]
        ):
            # predicted_values is a batch, extract the specific sample
            sample_preds = predicted_values[sample_idx]
        else:
            # predicted_values is already for a single sample
    ...
    if model_quantiles:
        ...
        try:
            ...
            logger.warning(
                "Could not find 0.1, 0.5, 0.9 quantiles for plotting. Plotting median only."
            )
