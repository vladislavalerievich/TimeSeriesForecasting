    ...
    def predict(self, dataset) -> List[SampleForecast]:
        ...
        for data_entry in dataset:
            try:
                ...
                input_data = data_entry[0] if isinstance(data_entry, tuple) else data_entry
                ...
                logger.error(f"Error processing data_entry: {type(data_entry)}, error: {str(e)}")
                raise
            ...
            future_values = torch.zeros((1, self.prediction_length, num_dims), dtype=torch.float32).to(self.device)

            start_np = start.to_timestamp().to_numpy()

            batch = BatchTimeSeriesContainer(history_values=history_values, future_values=future_values, start=start_np,
                                             frequency=frequency)

            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                with torch.no_grad():
                    output = self.model(batch, drop_enc_allow=False)
                    predictions = self.model.scaler.inverse_scale(output["result"], output["scale_statistics"])

            if torch.isnan(predictions).any():
                logger.warning(f"NaN predictions with half precision for {item_id}. Retrying with full precision.")
                with torch.no_grad():
                    output = self.model(batch, drop_enc_allow=False)
                    predictions = self.model.scaler.inverse_scale(output["result"], output["scale_statistics"])
            ...
            if torch.isnan(predictions).any():
                ...
                logger.warning(f"Predictions contain {nan_count}/{total_count} NaNs for {item_id}")
            ...
            forecast = SampleForecast(samples=samples, start_date=forecast_start, item_id=item_id)
...
class GiftEvaluator:
    ...
    def evaluate_datasets(self, datasets_to_eval: List[str], term: str, epoch: int = None, plot: bool = False):
        all_results = {}
        for ds_name in datasets_to_eval:
            ds_key, ds_freq = self._get_ds_key_freq(ds_name)
            task = {"ds_name": ds_name, "ds_key": ds_key, "ds_freq": ds_freq, "term": term}
            try:
                ...
                logger.error(f"Error evaluating {task['ds_name']} ({task['term']}): {str(e)}", exc_info=False)
                continue
        return all_results
    ...
    def _get_ds_key_freq(self, ds_name):
        if "/" in ds_name:
            ...
            ds_freq = self.dataset_properties_map.get(ds_key_mapped, {}).get("frequency", "D")
        ds_key = ds_key.lower()
        ds_key = PRETTY_NAMES.get(ds_key, ds_key)
        return ds_key, ds_freq

    def _evaluate_single_dataset(self, task: Dict) -> Tuple[Dict, List[SampleForecast], List[Dict]]:
        ds_name, ds_freq, term = task["ds_name"], task["ds_freq"], task["term"]

        is_multivariate = GiftEvalDataset(
        ...
        metrics_to_run = [
            MSE(forecast_type="mean"), MSE(forecast_type=0.5), MAE(forecast_type=0.5),
            MASE(forecast_type=0.5), MAPE(forecast_type=0.5), SMAPE(forecast_type=0.5),
            MSIS(), RMSE(forecast_type=0.5), RMSE(forecast_type="mean"),
            NRMSE(forecast_type=0.5), ND(forecast_type=0.5),
        ]
        if self.model.loss_type == 'quantile':
            metrics_to_run.append(MeanWeightedSumQuantileLoss(quantile_levels=self.model.quantiles))

        res = evaluate_model(self.predictor, test_data=test_data, metrics=metrics_to_run, axis=None, batch_size=128,
                             mask_invalid_label=True, allow_nan_forecast=False, seasonality=season_length)

        metrics = {
            "MSE[mean]": res["MSE[mean]"].iloc[0], "MSE[0.5]": res["MSE[0.5]"].iloc[0],
            "MAE[0.5]": res["MAE[0.5]"].iloc[0], "MASE[0.5]": res["MASE[0.5]"].iloc[0],
            "MAPE[0.5]": res["MAPE[0.5]"].iloc[0], "sMAPE[0.5]": res["sMAPE[0.5]"].iloc[0],
            "MSIS": res["MSIS"].iloc[0], "RMSE[0.5]": res["RMSE[0.5]"].iloc[0],
            "RMSE[mean]": res["RMSE[mean]"].iloc[0], "NRMSE[0.5]": res["NRMSE[0.5]"].iloc[0],
    ...
    def _plot_and_log_samples(self, plot_samples: List, forecasts: List[SampleForecast], task: Dict, epoch: int):
        ...
        try:
            ...
            # --- MODIFICATION START: Robustly handle tuple or dict data entries ---
            ...
            if history_values.ndim == 1: history_values = history_values.reshape(1, -1)
            if future_values.ndim == 1: future_values = future_values.reshape(1, -1)
            if history_values.shape[1] > max_context: history_values = history_values[:, -max_context:]
            ...
            if self.model.loss_type == 'quantile' and self.model.quantiles:
                try:
                    ...
                    lower_idx = q_list.index(0.1)
                    upper_idx = q_list.index(0.9)
                    ...
                    logger.warning(f"Could not find required quantiles for plotting {ds_name}. Plotting median only.")
            ...
            logger.warning(f"Failed to plot sample for {ds_name}/{term}: {e}", exc_info=False)