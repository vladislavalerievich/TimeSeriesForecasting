...
class ForecastPFNGeneratorWrapper(GeneratorWrapper):
    ...
    def _generate_spikes(
        ...
        """Generate spike effects for a time series."""
        ...
        if spikes_type in ["regular", "patchy"]:
            ...
            if spikes_type == "patchy":
                ...
                to_keep = np.random.randint(
                    np.ceil(patch_size * to_keep_rate), patch_size
                )
        else:
            n_spikes = (
                n_spikes
    ...
    def _apply_augmentations(
        ...
        if params.get("damp_and_spike", False):
            ...
            for idx in spike_indices:
                spikes = self._generate_spikes(batch_values.shape[1])
                if spikes.max() < 0:
                    batch_values[idx, :, :] = (
