...
from src.synthetic_generation.forecast_pfn_prior.series_config import SeriesConfig
...
def get_freq_component(
    dates_feature: pd.Index, n_harmonics: int, n_total: int, harmonic_scale: bool = True
):
    """
...
def make_series_seasonal(series: SeriesConfig, dates: pd.DatetimeIndex, options: dict):
    ...
    harmonic_scale = (
        (np.random.rand() < options["harmonic_scale_ratio"])
        if "harmonic_scale_ratio" in options
        else True
    )
    ...
    assert harmonic_rate > 0 and harmonic_rate < 3, (
        "Harmonic rate should be greater than 0 and less than 3"
    )
...
def make_series(
    ...
    if options["seasonal_only"]:
        ...
        trend_additional = (
            options["trend_additional"] if "trend_additional" in options else False
        )
